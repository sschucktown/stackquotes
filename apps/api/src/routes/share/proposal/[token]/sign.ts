...
import { createJobFromProposal } from "../../../../../lib/createJobFromProposal.js";
...
signRouter.post("/", async (c) => {
  ...
  const { data: proposal, error } = await supabase
    ...
    .select(
      "id, contractor_id, client_id, line_items, deposit_amount, status, signed_at"
    )
  ...
  if (proposal.status !== "sent") {
    return c.json({ error: "Proposal is not signable" }, 409);
  }
  ...
  const jobResult = await createJobFromProposal({
    supabase,
    proposal,
