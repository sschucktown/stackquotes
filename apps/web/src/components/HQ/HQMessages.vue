<template>
  <section class="space-y-3">
    <header class="flex items-center justify-between">
      ...
      <button
        type="button"
        class="inline-flex items-center gap-2 rounded-full border border-sky-100 bg-slate-50 px-3 py-1.5 text-sm font-medium text-slate-700 shadow-sm transition hover:shadow"
      >
        Inbox ->
      </button>
    </header>
    <div class="space-y-2">
      <article
        ...
        class="group flex items-center justify-between rounded-xl border border-slate-200 bg-white px-3.5 py-3.5 shadow-[0_1px_2px_rgba(0,0,0,0.04)] transition hover:shadow-sm"
      >
        <div class="flex items-start gap-3 overflow-hidden">
          <div class="relative">
            <div
              class="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-slate-200 to-slate-100 text-slate-700 text-sm font-semibold"
            >
              {{ thread.initials }}
            </div>
            <span
          ...
          <div class="min-w-0">
            ...
            <p class="mt-0.5 truncate text-sm text-slate-700">{{ thread.preview }}</p>
          </div>
        </div>
