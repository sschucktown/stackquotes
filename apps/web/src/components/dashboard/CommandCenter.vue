  ...
  <div class="min-h-screen bg-white">
    <div class="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      <transition name="fade-up">
        <div v-if="showAttentionBar" class="mt-3 rounded-lg bg-amber-50 px-3 py-2 text-sm text-amber-900 ring-1 ring-amber-200">
          {{ attentionMessage }}
        </div>
      </transition>
      ...
      <section v-if="!loading && !error" class="space-y-6">
        ...
        <div class="h-px bg-gradient-to-r from-slate-800/0 via-slate-800/40 to-slate-800/0" />
        <AiInsightsWidget :seen-not-accepted="insightsSeenNotAccepted" @cta="$emit('insights-cta')" />
      </section>

      <!-- Loading and Error States -->
      <section v-else class="space-y-6">
        <div v-if="loading" class="grid gap-3">
          <div class="animate-pulse rounded-2xl bg-slate-100 p-6 ring-1 ring-slate-200" />
          <div class="animate-pulse rounded-2xl bg-slate-100 p-16 ring-1 ring-slate-200" />
          <div class="animate-pulse rounded-2xl bg-slate-100 p-16 ring-1 ring-slate-200" />
        ...
        <div v-else class="rounded-2xl border border-rose-200 bg-rose-50 p-4 text-rose-900">
          ...
          <div class="mt-3 flex gap-2">
            ...
            <button type="button" class="rounded-md bg-rose-600 px-3 py-1.5 text-sm font-semibold text-white hover:bg-rose-500" @click="$emit('retry')">
              Retry
            </button>
          </div>
        </div>
