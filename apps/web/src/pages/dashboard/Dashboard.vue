<template>
  <div class="min-h-screen bg-gradient-to-b from-[#0f172a] to-[#1e293b]">
    <div class="mx-auto w-full max-w-5xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      <!-- Progress: Quotes → Proposals → Payments → Closed -->
      ...
      <section class="mt-6 space-y-6">
        ...
        <div v-if="hasAny(payments)" class="rounded-3xl bg-white p-4 shadow-sm">
          ...
          <div class="flex gap-3 overflow-x-auto pb-2 snap-x">
            <div
              ...
              :key="stage.key"
              v-if="payments[stage.key]?.length"
              class="min-w-[180px] snap-start rounded-xl border border-slate-200 bg-white p-3"
            >
              ...
              <TransitionGroup name="fade-up" tag="ul" class="mt-2 space-y-2">
                <li
                  v-for="item in payments[stage.key]"
                  :key="item.id"
                  class="rounded-lg bg-slate-50 p-2 text-sm text-slate-700"
                >
                  <p class="truncate font-medium">{{ item.title || 'Untitled' }}</p>
                  <p v-if="item.amount !== undefined" class="text-xs text-slate-500">
                    {{ currency(item.amount) }}
                  </p>
                  <button
                    type="button"
                    class="mt-2 w-full rounded-md bg-[#0EA5E9] px-2 py-1 text-xs font-semibold text-white"
                    @click="go(item.route)"
                  >
                    {{ stage.cta }}
                  </button>
        ...
        <div v-if="hasAny(smartProposals)" class="rounded-3xl bg-white p-4 shadow-sm">
          ...
          <div class="flex gap-3 overflow-x-auto pb-2 snap-x">
            <div
              v-for="stage in visibleProposalStages"
              :key="stage.key"
              v-if="smartProposals[stage.key]?.length"
              class="min-w-[180px] snap-start rounded-xl border border-slate-200 bg-white p-3"
            >
              <p class="text-xs font-semibold uppercase tracking-wide text-slate-500">{{ stage.label }}</p>
              <TransitionGroup name="fade-up" tag="ul" class="mt-2 space-y-2">
                <li
                  v-for="item in smartProposals[stage.key]"
                  :key="item.id"
                  class="rounded-lg bg-slate-50 p-2 text-sm text-slate-700"
                >
                  <p class="truncate font-medium">{{ item.title || 'Untitled' }}</p>
        ...
        <div v-if="hasAny(quickQuotes)" class="rounded-3xl bg-white p-4 shadow-sm">
          ...
          <div class="flex gap-3 overflow-x-auto pb-2 snap-x">
            <div
              v-for="stage in quoteStages"
              :key="stage.key"
              v-if="quickQuotes[stage.key]?.length"
              class="min-w-[180px] snap-start rounded-xl border border-slate-200 bg-white p-3"
            >
              <p class="text-xs font-semibold uppercase tracking-wide text-slate-500">{{ stage.label }}</p>
              <TransitionGroup name="fade-up" tag="ul" class="mt-2 space-y-2">
                <li
                  v-for="item in quickQuotes[stage.key]"
                  :key="item.id"
                  class="rounded-lg bg-slate-50 p-2 text-sm text-slate-700"
                >
                  <p class="truncate font-medium">{{ item.title || 'Untitled' }}</p>
    ...
    <button
      type="button"
      class="fixed bottom-5 right-5 inline-flex items-center justify-center rounded-full bg-[#0EA5E9] p-4 text-white shadow-lg ring-1 ring-white/20 hover:bg-[#10b3f0]"
      aria-label="New QuickQuote"
...
<style scoped>
/* Tailwind handles visual composition */
