...
<template>
  <div
    class="flex items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white px-3 py-2 shadow-[0_2px_10px_rgba(0,0,0,0.05)] transition-all duration-200"
    :class="selected ? 'ring-2 ring-emerald-200' : ''"
  >
    <div class="space-y-1">
    ...
    <div class="flex flex-col items-end gap-1 text-right">
      <span class="text-xs font-semibold text-slate-700">+{{ upgrade.priceDelta.toLocaleString("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 0 }) }}</span>
      <button
        type="button"
