  ...
  <div class="min-h-screen bg-slate-50 text-slate-900">
    <div class="mx-auto flex max-w-2xl flex-col gap-4 px-4 py-6 sm:px-6 pb-28">
            ...
            <div class="flex-1">
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                  <p class="text-sm font-semibold text-slate-900">{{ thread.clientName }}</p>
                  <span class="rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-semibold text-slate-600">
                    {{ thread.jobName }}
                  </span>
                </div>
                <span class="text-xs text-slate-500">{{ thread.lastUpdated }}</span>
              </div>
              <p class="text-sm text-slate-600">{{ thread.lastMessage }}</p>
            </div>
          </button>
        </div>
      </section>
      ...
      <section class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <div class="mb-3 flex items-center justify-between">
          <div>
            <p class="text-xs uppercase tracking-[0.08em] text-slate-500">Thread</p>
            <h2 class="text-lg font-semibold text-slate-900">{{ activeThread?.clientName }}</h2>
            <p class="text-xs text-slate-500">{{ activeThread?.jobName }}</p>
          </div>
          <span class="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">Live prototype</span>
        </div>

        <div class="max-h-[320px] space-y-3 overflow-y-auto rounded-xl bg-slate-50/70 p-3">
          <div
            v-for="message in activeMessages"
            :key="message.id"
            class="flex"
            :class="message.from === 'Contractor' ? 'justify-end' : 'justify-start'"
          >
            <div
              class="max-w-[80%] rounded-2xl px-4 py-3 text-sm shadow-sm sm:max-w-[70%]"
              :class="message.from === 'Contractor' ? 'bg-blue-600 text-white' : 'bg-white text-slate-800 border border-slate-200'"
            >
              <p class="leading-relaxed">{{ message.text }}</p>
              <p class="mt-2 text-[11px]" :class="message.from === 'Contractor' ? 'text-blue-50/90' : 'text-slate-500'">
                {{ message.time }} <span v-if="message.from === 'Contractor'"> · Sent</span>
              </p>
            </div>
          </div>
          <div v-if="!activeMessages.length" class="rounded-xl border border-dashed border-slate-200 bg-white px-4 py-6 text-center text-sm text-slate-500">
    ...
    <div class="fixed inset-x-0 bottom-0 border-t border-slate-200 bg-white/95 backdrop-blur">
      <div class="mx-auto flex max-w-2xl flex-col gap-2 px-4 py-3 sm:px-6">
        ...
        ></textarea>
        <div class="flex items-center justify-between">
          <span v-if="showSynced" class="text-xs font-semibold text-emerald-600">Synced ✓</span>
          <div class="flex items-center gap-2">
...
import { addMessageToThread, messageThreads } from "./usePrototypeEvents";
