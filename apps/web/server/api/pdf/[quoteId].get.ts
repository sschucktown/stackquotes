...
const { data: quote } = await sb.from('quotes').select('*, clients(*), contractors(*)').eq('id', quoteId).single()
...
<tbody>
${(q.options?.[t]||[]).map((i:any)=>`<tr><td>${i.description}</td><td>${i.qty}</td><td>$${i.unitPrice.toFixed(2)}</td><td>$${(i.qty*i.unitPrice).toFixed(2)}</td></tr>`).join('')}
</tbody>
</table>
</div>
`).join('')
...
${q.contractors?.logo_url ? `<img class="logo" src="${q.contractors.logo_url}"/>` : ''}
</div>
<div>
